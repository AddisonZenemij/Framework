CREATE TABLE `bd_az_framework`.`pd_daily_activities` (
	`id_pd_daily_activities` bigint(255) COLLATE utf8_unicode_ci NOT NULL COMMENT 'Id',
...
ALTER TABLE `bd_az_framework`.`pd_daily_activities`
	ADD PRIMARY KEY (`id_pd_daily_activities`),
	ADD KEY `IDX_id_pd_daily_activities` (`id_pd_daily_activities`),
...
ALTER TABLE `bd_az_framework`.`pd_daily_activities`
	MODIFY `id_pd_daily_activities` bigint(255) NOT NULL AUTO_INCREMENT;