	...
	class Sessions extends Models implements OCREND {
		...
		*/
		public function generate_session(int $id) {
			...
			$var_t_session['session'] = time() + SESSION_TIME;
			$this->db->update(t_az_user, $var_t_session, d_az_user_id_az_user . " = '" . $id . "'",'LIMIT 1');
		}
		/**
		...
		*/
		public function session_in_use(int $id) : bool {
			$time = time();
			if ($this->db->rows($this->db->query("SELECT " . d_az_user_id_az_user . " FROM " . t_az_user . " WHERE " . d_az_user_id_az_user . " = '" . $id . "' AND " . d_az_user_t_session . " >= '" . $time . "' LIMIT 1")) > 0) {
				return true;
			}
			return false;
		}
		...
		*/
		public function check_life(bool $force = false) {
			if (isset($_SESSION[SESS_APP_ID])) {
				...
				$time = time();
				if ($force || $this->db->rows($this->db->query("SELECT " . d_az_user_id_az_user . " FROM " . t_az_user . " WHERE " . d_az_user_id_az_user . " = '" . $id . "' AND " . d_az_user_t_session . " <= '" . $time . "' LIMIT 1")) > 0) {
					$var_t_session['session'] = 0;
					$this->db->update(t_az_user, $var_t_session, d_az_user_id_az_user . " = '" . $id . "'",'LIMIT 1');
