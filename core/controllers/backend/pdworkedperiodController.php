<?php
	...
	//------------------------------------------------
	class pdworkedperiodController extends Controllers {
		public function __construct() {
			...
			# Instanciando clases del modelo
			$var_m_pd_worked_period = new m_pd_worked_periodModel;
				...
				switch ($this->method) {
					case 'change':
						if (false != $var_dvdesign->sql_permits_t_value_change($var_controller)) {
							if ($this->isset_id) {
								...
								echo $this->template->render('backend/f_pd_worked_period/change', array(
									# Consultar registros de la tabla por medio del modelo instanciado
									'df_sql_controller_change' => $var_m_pd_worked_period->sql_controller_change(),
									# Consultar registros de la tabla por medio del modelo instanciado
					...
					case 'create':
						if (false != $var_dvdesign->sql_permits_t_value_create($var_controller)) {
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/f_pd_worked_period/create', array(
								# Cargar el controlador actual en una variable y ser declarada
								'df_var_controller' => $var_controller
							));
						} else {
					...
					case 'detail':
						if (false != $var_dvdesign->sql_permits_t_value_detail($var_controller)) {
							if ($this->isset_id) {
								# Cargar en el metodo por url la plantilla con su contenido a visualizar
								echo $this->template->render('backend/f_pd_worked_period/detail', array(
									# Consultar registros de la tabla por medio del modelo instanciado
									'df_sql_controller_detail' => $var_m_pd_worked_period->sql_controller_detail(),
									# Consultar registros de la tabla por medio del modelo instanciado
									'df_sql_permits_t_value_read_all' => $var_dvdesign->sql_permits_t_value_read_all($var_controller),
									# Cargar el controlador actual en una variable y ser declarada
									'df_var_controller' => $var_controller
					...
					case 'delete':
						if (false != $var_dvdesign->sql_permits_t_value_delete($var_controller)) {
							if ($this->isset_id) {
								# Cargar en el metodo por url la plantilla con su contenido a visualizar
								echo $this->template->render('backend/f_pd_worked_period/delete');
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'read/');
								# Detener la ejecucion
					...
					case 'eliminate':
						if (false != $var_dvdesign->sql_permits_t_value_read_eliminate($var_controller)) {
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/f_pd_worked_period/eliminate', array(
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_controller_eliminate' => $var_m_pd_worked_period->sql_controller_eliminate(),
								# Consultar registros de la tabla por medio del modelo instanciado
					...
					case 'read':
						if (false != $var_dvdesign->sql_permits_t_value_read_all($var_controller)) {
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/f_pd_worked_period/read', array(
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_controller_read' => $var_m_pd_worked_period->sql_controller_read(),
								# Consultar registros de la tabla por medio del modelo instanciado
					...
					case 'remove':
						if (false != $var_dvdesign->sql_permits_t_value_remove($var_controller)) {
							if ($this->isset_id) {
								# Cargar en el metodo por url la plantilla con su contenido a visualizar
								echo $this->template->render('backend/f_pd_worked_period/remove');
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'read/');
								# Detener la ejecucion
					...
					case 'update':
						if (false != $var_dvdesign->sql_permits_t_value_update($var_controller)) {
							if ($this->isset_id) {
								# Cargar en el metodo por url la plantilla con su contenido a visualizar
								echo $this->template->render('backend/f_pd_worked_period/update', array(
									# Consultar registros de la tabla por medio del modelo instanciado
									...
									'df_sql_controller_update' => $var_m_pd_worked_period->sql_controller_update(),
									# Cargar el controlador actual en una variable y ser declarada
									'df_var_controller' => $var_controller
								));
							} else {
